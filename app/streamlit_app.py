...
st.download_button("Download CSV", data=df.to_csv(index=False), file_name=f"ian-racing-model-{selected_date.isoformat()}.csv", mime="text/csv")
...
with st.expander("Component explanations", expanded=False):
    for score in scores:
        ...
        st.write({component.name: {"score": component.score, "confidence": component.confidence, "data_quality": component.data_quality, "explanation": component.explanation} for component in score.components})
