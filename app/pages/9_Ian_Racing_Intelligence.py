...
try:
    from racing_intelligence.tracking import v6_tracker_dataframe, v6_tracker_summary
except ImportError:
    def v6_tracker_dataframe(scores: list, meeting_date: date | None = None) -> pd.DataFrame:
        rows = []
        for race_scores in _v6_race_groups(scores).values():
    ...
    def v6_tracker_summary(df: pd.DataFrame) -> dict[str, str]:
        ...
        return {
            "v6_win_rate": _v6_ratio_text(int(win_rows["outcome"].eq("WIN").sum()), len(win_rows)),
            "v6_place_rate": _v6_ratio_text(
                int(place_rows["outcome"].isin(["WIN", "PLACED"]).sum()),
                len(place_rows),
            ),
        }

    def _v6_pick_row(item: Any, pick_type: str, meeting_date: date | None, race_scores: list) -> dict[str, Any]:
        runner = item.runner
        v6 = v6_analysis(item, race_scores)
        position = _v6_finish_position(runner.source_payload)
        place_cutoff = _v6_place_cutoff(runner.field_size)
        return {
            ...
            "outcome": _v6_pick_outcome(pick_type, position, place_cutoff),
            "v6_explanation": "; ".join(
                [
    ...
    def _v6_race_groups(scores: list) -> dict[tuple[str, str, str], list]:
    ...
    def _v6_pick_outcome(pick_type: str, position: int | None, place_cutoff: int) -> str:
        if position is None:
    ...
    def _v6_place_cutoff(field_size: int | None) -> int:
    ...
    def _v6_ratio_text(successes: int, total: int) -> str:
        if total == 0:
